...
; This model simulate residential movement on streets from the bottom-up
; by drawing a community-bubble of a acity
...
persons-own [start-patch]
;bubbles-own [bubble-radius]

; initialization
to setup
  ca ;; clears everything out to have a blank slate on which to start
  reset-ticks ;; reset timer

;  set streets gis:load-dataset "data/sf_queens.shp" ;; store the street shapefile data in the global variable called street-datasest
  set streets gis:load-dataset "data/street_pois_queens1500.shp"
  gis:set-world-envelope gis:envelope-of STREETS

;  foreach gis:feature-list-of STREETS [
;    x ->
;    ask patches gis:intersecting x [
;      set streets gis:property-value x "n_pois"
;      set pcolor blue
;    ]
;  ]
  ; git number of pois to patches
  foreach  gis:feature-list-of STREETS [
    [t]->
        ask patches gis:intersecting t [
          set streets gis:property-value t "n_pois"
          if gis:property-value t "n_pois" = 0 [set pcolor 76]
          if gis:property-value t "n_pois" = 1 [set pcolor 96]
          if gis:property-value t "n_pois" = 2 [set pcolor 94]
          if gis:property-value t "n_pois" = 3 [set pcolor 106]
          if gis:property-value t "n_pois" = 4 [set pcolor 116]
          if gis:property-value t "n_pois" = 5 [set pcolor 135]
          if gis:property-value t "n_pois" = 5 [set pcolor 126]
        ]
  ]
  ; set up agents
  setup-agents
...
to setup-agents
  ; agent flags
  create-flags 1 [setup-flag]

  ; agent persons
  create-ordered-persons num-person [setup-person]

  ; agent bubbles
  create-ordered-bubbles 20 [setup-bubble]
end
...
to setup-person
  ...
  set size 4
  set color yellow
  set start-patch patch-here ; set initial location as their initial patch
;  pen-down
end
...
; what happens each step of the model
to go
  ask persons [
    random-walk
  ]

  ;let peak-density-person max-one-of persons [ count persons with [ distance myself < 100] ]
  ;let bubble-radius [distance start-patch] of peak-density-person
  let bubble-radius mean [distance start-patch] of persons
;  show bubble-radius
  ask bubbles [
    fd (pi * bubble-radius / 180) * 20
    rt 90
  ]
end
...
;    [set pcolor 9]
;    if (pxcor >= -9 and pxcor <= 17) and (pycor < 11 and pycor > 9)
;    [set pcolor 9]

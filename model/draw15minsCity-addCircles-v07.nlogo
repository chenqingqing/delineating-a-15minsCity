...
to turn-until-satisfy
 ...
 ; let unvisited patches with [ not member? self [memory] of myself and is-accessible? ]


;  let accessible-patches patches with [ is-accessible? ]
;  let unvisited accessible-patches with [ not member? self [memory] of myself ]
;  set target one-of neighbors with [ is-accessible? ]
  ;let target one-of neighbors with-max [POIs]
  let seen? member? target memory

  while [ seen? ] [rt random 360 turn-until-satisfy ]
;  if ( n < 7 )[
;    if ( member? target [ memory ] of myself) []
;  ]
;  if (member? target [ memory ] of myself)[
;  show target
;  show memory]
...
;  if n < 7 [ if seen? [ turn-until-satisfy n + 1 ]]
  ; move to target
;  face target
  move-to target

...
to setup
  ...
  ;;set streets gis:load-dataset "data/sf_queens.shp"
...
to setup-agents
  ...
  ; agent bubbles
  create-ordered-bubbles 10 [setup-bubble]
end
...
to go
  ...
  ;set movement
  ask persons [random-walk]
  show [who] of persons with [color = yellow]
...
;to random-walk
;  let n_neighbor count neighbors with [pcolor != black]
;  (ifelse
;    n_neighbor > 1 [
;
;    ]
...
to random-walk
  ...
  move-to one-of unvisited in-radius 10
  set memory lput patch-here memory
end
...
;  move-to target
;  set memory lput patch-here memory
;  ;ask patch-here [ set pcolor orange ] ; just to show what's visited
...
;
;  if n < 8
;  [if seen? [turn-until-satisfy n + 1]]
;
;
;
;;  set target ifelse-value (patch-ahead 1 = patch-here)
;;    [ patch-ahead 2]
;;    [ patch-ahead 1]
;;
;;  let seen? member? target memory
;;  let not-accessible? [ not is-accessible? ] of target
;;
;;  if-else n < 8
;;  [if seen? or not-accessible? [turn-until-satisfy n + 1]]
;;  [if not-accessible? [turn-until-satisfy n + 1]]
;
;
;
;

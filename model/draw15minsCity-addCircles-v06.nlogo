...
; to set up agents------------------------------------------------------------------------
to setup-agents
;  ; agent: x and y axises
;  create-axises 1 [setup-axises]
;
;  ; agent: background circle
;  let n_ring ceiling (world-width * sqrt 2 / (ring-radius * 2) )
;  show world-width
;  show n_ring
;  create-rings n_ring [setup-rings]
...
;  diffuse heat 1
;  ask persons [
;    ask patch-here [set pcolor heat ]
;  ]
...
to random-walk
  ...
  if show-patch? [ ask patch-here [set pcolor white ] ] ; to show what is visited
...
to turn-until-satisfy [n]
  ...
  set target one-of neighbors with [ is-accessible? ]

  let seen? member? target memory

  if n < 8
  [ if seen?
    [lt 90 turn-until-satisfy n + 1]]
end
; --------------------------------------------------------------------------------------------------------------------
...
"" ""
PENS
"Inside" 1.0 1 -1184463 true "" "histogram [distance START-PATCH] of persons with [ distance START-PATCH  < BUBBLE-RADIUS ]"
